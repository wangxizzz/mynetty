 ...
 * @author wangxi
 */
public class RpcNioMultiClient {
    private static RpcNioMultiClient rpcNioClient;
    ...
    public static RpcNioMultiClient getInstance() {
        if (rpcNioClient == null) {
            synchronized (RpcNioMultiClient.class) {
                if (rpcNioClient == null) {
                    rpcNioClient = new RpcNioMultiClient();
                }
            }
        }
        return rpcNioClient;
    ...
    public boolean sendMsg2Server(byte[] bytes) {
        try {
            ByteBuffer buffer = ByteBuffer.allocate(bytes.length + 4);  // 4表示消息的长度，用于协议的反序列化
            // 放入消息长度，然后放入消息体
